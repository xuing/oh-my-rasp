...
image="${OHMYRASP_VULHUB_JBOSS_JMX_IMAGE:-vulhub/jboss:as-6.1.0}"
baseline_name="${OHMYRASP_VULHUB_JBOSS_JMX_BASELINE_NAME:-ohmyrasp-vulhub-jbossjmx-baseline}"
protected_name="${OHMYRASP_VULHUB_JBOSS_JMX_PROTECTED_NAME:-ohmyrasp-vulhub-jbossjmx-protected}"
baseline_port="${OHMYRASP_VULHUB_JBOSS_JMX_BASELINE_PORT:-19262}"
protected_port="${OHMYRASP_VULHUB_JBOSS_JMX_PROTECTED_PORT:-19263}"
ysoserial_jar="${OHMYRASP_YSOSERIAL_JAR:-/tmp/ohmyrasp-ysoserial/ysoserial.jar}"
success_file="/tmp/ohmyrasp-jbossjmx-success"
baseline_dir="logs/vulhub-jboss-jmxinvoker-java7-baseline"
protected_dir="logs/vulhub-jboss-jmxinvoker-java7-protected"
...
generate_payload() {
  ...
  if [[ ! -s "$output" ]]; then
    echo "ysoserial did not generate a JBoss JMXInvokerServlet payload" >&2
    exit 1
  fi
}
...
send_payload() {
  ...
  curl -sS -o "$output" -w "%{http_code}" \
    ...
    --data-binary "@${payload}" \
    "http://127.0.0.1:${port}/invoker/JMXInvokerServlet" || true
}
...
status="$(send_payload "$baseline_port" "$payload_file" "${baseline_dir}/jmxinvoker.response")"
printf 'jmxinvoker_status=%s\n' "$status" >> "${baseline_dir}/attempts.log"
if ! docker exec "$baseline_name" sh -c "test -e '${success_file}'"; then
  sed -n '1,180p' "${baseline_dir}/jmxinvoker.response" >&2 || true
  docker logs "$baseline_name" >&2 || true
  echo "baseline JBoss JMXInvokerServlet did not create ${success_file}" >&2
  exit 1
fi
printf 'jmxinvoker_marker=present\n' >> "${baseline_dir}/attempts.log"
...
if ! grep -q 'Unsupported major.minor version 52.0' "${protected_dir}/container.log"; then
  ...
  echo "JBoss JMXInvokerServlet Java 7 protected probe did not show Java 8 agent class-version mismatch" >&2
  exit 1
fi
if docker ps --filter "name=${protected_name}" --filter "status=running" --format '{{.Names}}' | grep -q .; then
  echo "JBoss JMXInvokerServlet Java 7 container unexpectedly kept running with Java 8 agent" >&2
  exit 1
fi

echo "vulhub JBoss JMXInvokerServlet Java7 legacy boundary passed"
