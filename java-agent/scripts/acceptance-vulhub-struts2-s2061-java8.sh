...
image="${OHMYRASP_VULHUB_STRUTS2_S2061_IMAGE:-vulhub/struts2:2.5.25}"
baseline_name="${OHMYRASP_VULHUB_STRUTS2_S2061_BASELINE_NAME:-ohmyrasp-vulhub-struts2-s2061-baseline}"
protected_name="${OHMYRASP_VULHUB_STRUTS2_S2061_PROTECTED_NAME:-ohmyrasp-vulhub-struts2-s2061-protected}"
baseline_port="${OHMYRASP_VULHUB_STRUTS2_S2061_BASELINE_PORT:-18542}"
protected_port="${OHMYRASP_VULHUB_STRUTS2_S2061_PROTECTED_PORT:-18543}"
host_agent_jar="$(pwd)/agent-java8/build/libs/ohmyrasp-agent-java8.jar"
baseline_dir="logs/vulhub-struts2-s2-061-java8-baseline"
protected_dir="logs/vulhub-struts2-s2-061-java8-protected"
...
wait_for() {
  ...
  echo "${name} did not expose Struts2 S2-061 root at ${port}" >&2
  exit 1
}
...
headers = {
    ...
    "User-Agent": "ohmyrasp-s2-061",
}
connection = http.client.HTTPConnection("127.0.0.1", port, timeout=30)
try:
    connection.request("POST", "/index.action", body=body, headers=headers)
...
if ! grep -q '"event":"ohmyrasp-java8-agent-start"' "$protected_log"; then
  ...
  echo "missing Java 8 startup event in Struts2 S2-061 protected container" >&2
  exit 1
fi
if grep -q '"event":"ohmyrasp-detection"' "$protected_log"; then
  cat "$protected_log" >&2
  echo "Struts2 S2-061 protected startup/readiness produced a detection before the exploit request" >&2
  exit 1
fi
...
if [[ "$baseline_status" != "200" ]] || ! grep -q 'uid=0(root)' "${baseline_dir}/attack.response"; then
  ...
  echo "baseline Struts2 S2-061 multipart OGNL did not return id output; status=${baseline_status}" >&2
  exit 1
fi
...
if grep -q 'uid=0(root)' "${protected_dir}/attack.response"; then
  ...
  echo "protected Struts2 S2-061 multipart OGNL returned command output despite Java8 RASP; status=${protected_status}" >&2
  exit 1
fi
for _ in $(seq 1 30); do
  if grep -q '"algorithm":"java8_command_execution_exploit_primitive".*"action":"block"' "$protected_log"; then
    echo "vulhub Struts2 S2-061 Java8 acceptance passed"
    exit 0
  fi
  sleep 1
done
...
cat "$protected_log" >&2
echo "missing java8_command_execution_exploit_primitive block event for Struts2 S2-061" >&2
exit 1
