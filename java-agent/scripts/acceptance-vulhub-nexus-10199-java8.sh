...
image="${OHMYRASP_VULHUB_NEXUS_10199_IMAGE:-vulhub/nexus:3.21.1}"
baseline_name="${OHMYRASP_VULHUB_NEXUS_10199_BASELINE_NAME:-ohmyrasp-vulhub-nexus10199-baseline}"
protected_name="${OHMYRASP_VULHUB_NEXUS_10199_PROTECTED_NAME:-ohmyrasp-vulhub-nexus10199-protected}"
baseline_port="${OHMYRASP_VULHUB_NEXUS_10199_BASELINE_PORT:-18685}"
protected_port="${OHMYRASP_VULHUB_NEXUS_10199_PROTECTED_PORT:-18686}"
marker="${OHMYRASP_VULHUB_NEXUS_10199_MARKER:-/tmp/ohmyrasp-nexus10199-success}"
admin_password_file="${OHMYRASP_VULHUB_NEXUS_10199_ADMIN_PASSWORD_FILE:-/tmp/vulhub-ohmyrasp-20260603/nexus/CVE-2020-10199/admin.password}"

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd -- "${script_dir}/../.." && pwd)"
agent_root="${repo_root}/java-agent"
agent_jar="${agent_root}/agent-java8/build/libs/ohmyrasp-agent-java8.jar"
work_dir="${OHMYRASP_VULHUB_NEXUS_10199_WORK_DIR:-/tmp/ohmyrasp-nexus10199}"
baseline_response="${work_dir}/baseline-response.txt"
protected_response="${work_dir}/protected-response.txt"
baseline_repo_response="${work_dir}/baseline-repositories.txt"
protected_repo_response="${work_dir}/protected-repositories.txt"
...
read_repositories() {
  ...
  status="$(
    curl -sS \
      ...
      -o "${response_file}" \
      -w '%{http_code}' \
      "http://127.0.0.1:${port}/service/rest/v1/repositories"
  )"
  if [[ "${status}" != "200" ]] || ! grep -q '"name"' "${response_file}"; then
    echo "Nexus repository read failed on port ${port}; status=${status}" >&2
...
send_exploit() {
  ...
  status="$(
    curl -sS \
      ...
      -w '%{http_code}' \
      --data-binary "${payload}" \
      "http://127.0.0.1:${port}/service/rest/beta/repositories/go/group"
  )"
  if [[ "${status}" != "400" && "${status}" != "201" ]]; then
    echo "Nexus CVE-2020-10199 exploit request failed on port ${port}; status=${status}" >&2
    cat "${response_file}" >&2 || true
    return 1
  fi
}
...
read_repositories "${baseline_port}" "${baseline_repo_response}"
read_repositories "${protected_port}" "${protected_repo_response}"
...
if ! docker exec "${baseline_name}" test -f "${marker}"; then
  echo "baseline Nexus CVE-2020-10199 exploit did not create marker" >&2
  cat "${baseline_response}" >&2 || true
  docker logs --tail 120 "${baseline_name}" >&2 || true
  exit 1
fi

if ! grep -q 'UNIXProcess' "${baseline_response}"; then
  echo "baseline Nexus CVE-2020-10199 response did not show evaluated process object" >&2
  cat "${baseline_response}" >&2 || true
  exit 1
fi
...
if [[ "${after_count}" -le "${before_count}" ]]; then
  echo "missing java8_el_runtime_execution block event for Nexus CVE-2020-10199" >&2
  cat "${protected_log}" >&2 || true
...
if docker exec "${protected_name}" test -f "${marker}"; then
  echo "protected Nexus CVE-2020-10199 exploit created marker despite RASP" >&2
  cat "${protected_log}" >&2 || true
  exit 1
fi

if grep -q 'UNIXProcess' "${protected_response}"; then
  echo "protected Nexus CVE-2020-10199 response still exposed an evaluated process object" >&2
  cat "${protected_response}" >&2
  exit 1
fi

if grep -q '"algorithm":"java8_command_execution_exploit_primitive"' "${protected_log}"; then
  echo "Nexus CVE-2020-10199 reached Runtime.exec instead of stopping at EL evaluation" >&2
  cat "${protected_log}" >&2
  exit 1
fi

if grep -q 'ohmyrasp-nexus10199-success' "${protected_log}"; then
  echo "protected EL event leaked the raw Nexus payload marker" >&2
  cat "${protected_log}" >&2
  exit 1
fi

echo "vulhub Nexus CVE-2020-10199 Java8 acceptance passed"
